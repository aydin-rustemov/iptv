...
export class CanlitvMeAdapter implements SourceAdapter {
  name = "canlitv-me";
  private baseUrl: string;

  constructor(baseUrl = "https://www.canlitv.me/live") {
    ...
    try {
      ...
      for (const channelUrl of pagesToVisit) {
        try {
          ...
          if (res.ok) {
            ...
            const chName = cheerio.load(html)("title").text() || "CanliTV Me Channel";
      ...
      if (pendingPages.length > 0) {
        ...
        try {
          ...
          const context = await createSafeContext(browser, ["canlitv.me"]);
          ...
          for (const channelUrl of pendingPages.slice(0, 5)) {
            ...
            try {
              ...
              const norm = normalizeChannel(title || "CanliTV Me Channel");
